...
function saveData() {
  localStorage.setItem("storyvault_universities", JSON.stringify(universities));
  localStorage.setItem("storyvault_stories", JSON.stringify(stories));
}

function loadData() {
  const savedUniversities = localStorage.getItem("storyvault_universities");
  const savedStories = localStorage.getItem("storyvault_stories");

  if (savedUniversities !== null) {
    universities = JSON.parse(savedUniversities);
  }
  ...
  nextUniId =
    universities.length > 0
      ? Math.max(...universities.map(u => u.id)) + 1
      : 1;
